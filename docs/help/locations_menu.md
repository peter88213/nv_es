[novelibre ${Home page}](https://github.com/peter88213/novelibre) > [Páginas en castellano](../) > [${Online help}](./) > Lugares menu

---

# Lugares menu

**Location operation**

## Añadir

**Add a new location**

With **Lugares > Añadir**, you can add a location to the tree.

-   If a location is selected, the new location is placed after the
    selected one.
-   Otherwise, the new location is placed at the last position.
-   The new location has an auto-generated title. You can change it in
    the right pane.

---

## ${Assign color...}

**Assign a color to all selected locations**

With **Lugares > ${Assign color...}**
you can assign a color to all locations in a single or multiple selection.
A color picker dialog opens.
If you cancel the color choosing process, the color of the selected locations remain unchanged.


## ${Reset color}

**Reset the colors of all selected locations**

With **Lugares > ${Reset color}**
you can clear the color assignments for all locations in a single or multiple selection.

---

## ${Import}

**Import locations from another project**

With **Lugares > ${Import}**, you can import a selection of locations
from another project. First you select an XML file containing the
location data. Then you select the locations you want to add to the
current project.

> **Hint**
> 
> To create an XML location data file for the current project, use
> **${Export} > ${XML data files}**.

---

## ${Export location descriptions for editing}

**Export an ODT document that can be imported again after editing**

With **Objetos > ${Export location descriptions for editing}**, you can
create a text document that contains location descriptions that can be
edited with *Writer* and reimported. File name suffix is
`_locations_tmp`.

## ${Export location table}

**Export an ODS document that can be imported again after editing**

With **Objetos > ${Export location table}**, you can create a spreadsheet
that contains a row per location. The document can be edited with *Calc*
and reimported. File name suffix is `_loclist_tmp`.

> **Note**
> 
> You can reorder, hide or delete columns and rows without affecting the
> reimport. Only the first column and the first row, which are hidden by
> default, must not be changed as they contain the structural information
> for the import.

---

## ${Show table in Browser}

**Show an HTML report with locations data**

With **Lugares > ${Show table in Browser}**, you can create a HTML file
that contains a location table, and launch your system's web browser for
displaying it.

> **Note**
> 
> The report is a temporary file, auto-deleted on program exit. If needed,
> you can have your web browser save or print it.

---

[English manual](https://peter88213.github.io/nvhelp-en/)


Copyright (c) by Peter Triesberger. All rights reserved.
