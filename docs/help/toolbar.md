[novelibre ${Home page}](https://github.com/peter88213/novelibre) > [Páginas en castellano](../) > [${Online help}](./) > The toolbar

---

# The toolbar

The toolbar provides buttons for common actions in the proposed
workflow.

---

![${Back}](images/goBack.png) Go back in the browsing history. Same as
`Alt`-`Left`.

![${Forward}](images/goForward.png) Go forward in the browsing history.
Same as `Alt`-`Right`.

---

![${Show Book}](images/viewBook.png) Go to the "${Book}" branch and expand
it. Same as **Ver > ${Show Book}**.

![${Show Plot lines}](images/viewArcs.png) Go to the "${Plot lines}" branch
and expand it. Same as **Ver > ${Show Plot lines}**.

![${Show Characters}](images/viewCharacters.png) Go to the "Caracteres"
branch and expand it. Same as **Ver > ${Show Characters}**.

![${Show Locations}](images/viewLocations.png) Go to the "Lugares"
branch and expand it. Same as **Ver > ${Show Locations}**.

![${Show Items}](images/viewItems.png) Go to the "Objetos" branch and
expand it. Same as **Ver > ${Show Items}**.

![${Show Project notes}](images/viewProjectnotes.png) Go to the "${Project notes}" 
branch and expand it. Same as **Ver > ${Show Project notes}**.

---

![Guardar](images/save.png) Save the project. Same as **Archivo > Guardar** or
`${Ctrl}`-`S`.

![${Lock}/${Unlock}](images/lock.png) Toggle the lock status of the project.

![${Update from manuscript}](images/updateFromManuscript.png) Import the
current manuscript. Same as selecting the manuscript under **${Import}**.

![Export manuscript](images/manuscript.png) Export the manuscript for
editing. Same as **${Export} > ${Manuscript for editing}**, but without
confirmation for opening the document.

---

![Añadir](images/add.png) Add element. Same as `${Ctrl}`-`N`.

![${Add child}](images/addChild.png) Add child element. Same as
`${Ctrl}`-`Alt`-`N`.

![${Add parent}](images/addParent.png) Add element on the parent's level.
Same as `${Ctrl}`-`Alt`-`${Shift}`-`N`.

![Borrar](images/remove.png) Delete selected elements. Same as `${Del}`.

---

![Cortar](images/cut.png) Move the selected element from the tree to the
clipboard. Same as `${Ctrl}`-`X`.

![Copiar](images/copy.png) Copy the selected element to the clipboard.
Same as `${Ctrl}`-`C`.

![Pegar](images/paste.png) Paste the element stored in the clipboard to
the tree. Same as `${Ctrl}`-`V`.

You can copy and paste the following tree elements via the system
clipboard:

-   Parts and chapters,
-   sections,
-   stages,
-   plot lines,
-   plot points,
-   characters,
-   locations,
-   items,
-   project notes.

> **Hint**
> 
> 
> If multiple elements are selected, only the first one will be copied.
> However, if the element has "children", these will also be copied and
> pasted.


> **Attention**
>
>
> Relationships are not included when copying to the clipboard. This also
> applies to the section viewpoint and for plot lines/plot points.

---

![${Highlight tagged elements}](images/tag.png) 
Highlight elements with a specified tag. This opens a tag selection dialog.
Same as **Ver > ${Highlight tagged elements}**.

![${Select highlighted elements}](images/select.png) 
Select all highlighted elements in the project tree for further action.

![${Reset Highlighting}](images/reset_highlight.png) Reset the highlighting of tree elements.
Same as **Ver > ${Reset Highlighting}**.

While tree elements are highlighted, the criterion is displayed on the right of the reset button.

---

![${Toggle Text viewer}](images/viewer.png) Toggle Text viewer. Same as
**Ver > ${Toggle Text viewer}** or `${Ctrl}`-`T`.

![${Toggle Properties}](images/properties.png) Toggle Properties. Same as
**Ver > ${Toggle Properties}** or `${Ctrl}`-`Alt`-`T`.


---

[English manual](https://peter88213.github.io/nvhelp-en/)


Copyright (c) by Peter Triesberger. All rights reserved.
